...
<div class="container mx-auto my-10 p-8 bg-white shadow-lg rounded-lg">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('create') }}" class="btn-primary text-white font-bold py-2 px-4 rounded">
            Tambah Product
        </a>
    </div>
            ...
            <tbody class="text-gray-600 text-sm font-light">
                @if ($products->count() > 0)
                    @foreach ($products as $index => $product)
                        <tr class="product-row border-b border-gray-200 hover:bg-gray-100 {{ $product['stock'] <= 3 ? 'low-stock' : '' }}">
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center">
                                    <a href="{{ route('edit', $product['id']) }}" class="btn-info text-white font-bold py-1 px-3 rounded mr-2">Edit</a>
                                    <form action="{{ route('delete', $product['id']) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit" class="btn-danger text-white font-bold py-1 px-3 rounded">Delete</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
